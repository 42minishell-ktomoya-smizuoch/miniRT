...
/*   Updated: 2024/06/19 15:27:55 by smizuoch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	calculate_discriminant(t_metal *metal, t_ray *ray, t_limits l,
								t_hit_metal *m)
{
...
static void	fill_hit_record(t_hit_record *rec, t_hit_metal *m, t_metal *metal,
							t_ray *ray)
{
	rec->t = m->root;
...
int	hit_metal(t_hittable *self, t_ray *ray, t_limits l, t_hit_record *rec)
{
	...
	if (!calculate_discriminant(metal, ray, l, &m))
		return (0);
	fill_hit_record(rec, &m, metal, ray);
	return (1);
}
